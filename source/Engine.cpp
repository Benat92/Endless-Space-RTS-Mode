...
void Engine::Step(bool isActive)
{
	...
	// Update this here, for thread safety. Old Code below
...
if(!selectedShip->HasTravelPlan() && selectedShip && selectedShip->GetTargetSystem())
		selectedShip->TravelPlan().push_back(selectedShip->GetTargetSystem());
	if(selectedShip->HasTravelPlan() && currentSystem == selectedShip->TravelPlan().back())
