	...
	acceptedIt(player.AvailableJobs().empty() ? accepted.begin() : accepted.end())
{
	...
	//RTS mode changed code from player.TravelPlan to selectedShip.Travel pla. 2 times.
	if(!FindMissionForSystem(selectedSystem) && selectedShip->HasTravelPlan())
	{
		const auto &tp = selectedShip->TravelPlan();
