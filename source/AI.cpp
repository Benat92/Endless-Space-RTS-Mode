...
void AI::Step( PlayerInfo &player)
{
...
void AI::MovePlayer(Ship &ship, PlayerInfo &player)
{
	Command command;

	//RTS mode added *selectedShip reference
    Ship *selectedShip = player.Flagship();

	bool isWormhole = false;
	//RTS mode changed player.HasTravelPlan & player.TravelPlan to ship element
	if(ship.HasTravelPlan())
	{
		const System *system = ship.TravelPlan().back();
		for(const StellarObject &object : ship.GetSystem()->Objects())
