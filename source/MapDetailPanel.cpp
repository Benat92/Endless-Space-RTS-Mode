...
{
    //RTS mode addition
    Ship *selectedShip = player.Flagship();



	if((key == SDLK_TAB || command.Has(Command::JUMP)) && selectedShip)
	{
		...
		const System *source = plan.empty() ? selectedShip->GetSystem() : plan.front();
		...
		if(!plan.empty() && !(mod & KMOD_SHIFT))
		{
			...
			next = source;
			source = plan.empty() ? selectedShip->GetSystem() : plan.front();
		...
		bool hasJumpDrive = selectedShip->Attributes().Get("jump drive");
...
void MapDetailPanel::DrawOrbits()
{
	...
	const Sprite *orbitSprite = SpriteSet::Get("ui/description panel");
	Point orbitCenter(Screen::Right() - 120, Screen::Bottom() - 150);
